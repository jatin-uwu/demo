...
service ITSMService {
    ...
    /*=====================================================
        ...
        The generic header lives here and the incident-specific
        fields live on IncidentForm, reached through the
        `incidentForm` composition. Clients that need both read
        them with $expand=incidentForm and write them with a
        deep insert/update.
    ...
    entity Tickets as projection on txn.Ticket excluding { history } actions {
        ...
        /*-------------------------------------------------
            ...
            the DRAFT -> NEW transition. on('UPDATE') refuses
    ...
    // Deliberately NOT Capabilities-locked like its sibling child entities
    // below: unlike them, we already have a working upload flow (Main
    // controller) that creates the Attachment metadata row with a
    // standalone POST — to get its ID — and only then streams the file's
    // bytes to it with a plain PUT to /Attachments(ID)/content. Locking
    // Insertable/Updatable to false the way the reference does for entities
    // it has no UI for yet would break that flow.
